...
type Config struct {
	...
	// Embedding provider selection: "openrouter" or "ollama" (default).
	// When "openrouter", reads OPENROUTER_API_KEY and EMBEDDING_MODEL env vars
	// as defaults for the embed client (MNEMO_EMBED_* vars still override).
...
func Load() (*Config, error) {
	...
	cfg := &Config{
		...
		EmbedDims:             envInt("MNEMO_EMBED_DIMS", 0), // 0 = unset, resolved below based on provider
	...
	// Apply provider-based embedding defaults.
	// MNEMO_EMBED_* vars take precedence; provider fills in the gaps.
	switch cfg.EmbedProvider {
	case "openrouter":
		if cfg.EmbedBaseURL == "" {
			cfg.EmbedBaseURL = "https://openrouter.ai/api/v1"
		}
		if cfg.EmbedAPIKey == "" {
			cfg.EmbedAPIKey = os.Getenv("OPENROUTER_API_KEY")
		}
		if cfg.EmbedModel == "" {
			cfg.EmbedModel = envOr("EMBEDDING_MODEL", "openai/text-embedding-3-small")
		}
		if cfg.EmbedDims == 0 {
			cfg.EmbedDims = 1536
		}
	case "ollama", "":
		// Ollama / local embedding path — reads from MNEMO_EMBED_* only.
		if cfg.EmbedDims == 0 {
			cfg.EmbedDims = 256
		}
	default:
		return nil, fmt.Errorf("unsupported EMBEDDING_PROVIDER %q; valid values are \"openrouter\" and \"ollama\"", cfg.EmbedProvider)
	}

	// Validate embed dimensions are within a reasonable range.
	if cfg.EmbedDims < 1 || cfg.EmbedDims > 4096 {
		return nil, fmt.Errorf("MNEMO_EMBED_DIMS must be between 1 and 4096; got %d", cfg.EmbedDims)
	}
