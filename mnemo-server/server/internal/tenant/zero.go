...
type ZeroProvisioner struct {
	...
	autoModel  string
	autoDims   int
	clientDims int // VECTOR column width for client-side embedding (non-auto mode)
	ftsEnabled bool
}

// NewZeroProvisioner creates a provisioner for TiDB Zero API.
// backend is "tidb", "postgres", or "db9".
// clientDims is the VECTOR column width used when auto-embedding is not active.
func NewZeroProvisioner(baseURL, backend, autoModel string, autoDims, clientDims int, ftsEnabled bool) *ZeroProvisioner {
