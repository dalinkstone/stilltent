...
// bulkBatchSize controls how many rows are batched into a single multi-value
// INSERT statement. This reduces round-trips to TiDB dramatically compared to
// inserting one row at a time. 50 is conservative enough to stay well under
// the max_allowed_packet limit even with large MEDIUMTEXT content.
...
func (r *MemoryRepo) BulkCreate(ctx context.Context, memories []*domain.Memory) error {
	...
	// Process in batches to reduce round-trips while staying under packet limits.
	for batchStart := 0; batchStart < len(memories); batchStart += bulkBatchSize {
		batchEnd := batchStart + bulkBatchSize
		if batchEnd > len(memories) {
			batchEnd = len(memories)
		}
		batch := memories[batchStart:batchEnd]

		if err := r.bulkInsertBatch(ctx, tx, batch); err != nil {
			return err
		}
	}
	return tx.Commit()
...
// bulkInsertBatch inserts a single batch of memories using a multi-value INSERT.
func (r *MemoryRepo) bulkInsertBatch(ctx context.Context, tx *sql.Tx, batch []*domain.Memory) error {
	var columns string
	var placeholderRow string
	if r.autoModel != "" {
		columns = `(id, content, source, tags, metadata, memory_type, agent_id, session_id, state, version, updated_by, created_at, updated_at)`
		placeholderRow = `(?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, NOW(), NOW())`
	} else {
		columns = `(id, content, source, tags, metadata, embedding, memory_type, agent_id, session_id, state, version, updated_by, created_at, updated_at)`
		placeholderRow = `(?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, NOW(), NOW())`
	}

	// Build multi-value INSERT: INSERT INTO memories (...) VALUES (...), (...), ...
	var sb strings.Builder
	sb.WriteString("INSERT INTO memories ")
	sb.WriteString(columns)
	sb.WriteString(" VALUES ")

	args := make([]any, 0, len(batch)*11) // pre-allocate for ~11 params per row
	for i, m := range batch {
		if i > 0 {
			sb.WriteByte(',')
		}
	...
	if err != nil {
		...
		if errors.As(err, &mysqlErr) && mysqlErr.Number == 1062 {
			return fmt.Errorf("bulk insert: %w", domain.ErrDuplicateKey)
		}
		return fmt.Errorf("bulk insert batch: %w", err)
	}
	return nil
}
...
//
// IMPORTANT: VEC_COSINE_DISTANCE(embedding, ?) must appear identically
// (byte-for-byte) in both SELECT and ORDER BY. This is a TiDB requirement
// for the HNSW vector index to be used. Using an alias like
// "ORDER BY distance" would bypass the vector index and fall back to a
// full brute-force scan. The duplicate parameter binding is intentional.
...
// Uses strconv.FormatFloat instead of fmt.Sprintf for ~3x better throughput
// on hot-path vector serialization (avoids fmt reflection overhead).
func vecToString(embedding []float32) any {
	if len(embedding) == 0 {
		return nil
	}
	var sb strings.Builder
	sb.Grow(len(embedding)*12 + 2) // pre-allocate: ~10 chars per float + comma + brackets
	sb.WriteByte('[')
	for i, v := range embedding {
		if i > 0 {
			sb.WriteByte(',')
		}
		sb.WriteString(strconv.FormatFloat(float64(v), 'g', 8, 32))
	}
	sb.WriteByte(']')
	return sb.String()
}
