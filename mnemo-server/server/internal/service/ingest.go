...
Analyze the new facts and determine whether each should be added, updated, or deleted in memory. Return the full memory state after reconciliation.`, string(refsJSON), string(factsJSON))
	...
	// Step 4: Pre-compute embeddings for all ADD/UPDATE texts in a single batch
	// to eliminate per-event HTTP round-trips. Collect texts that need embeddings
	// first, batch-embed them, then execute the actions using pre-computed vectors.
	type pendingAction struct {
		event     reconcileEvent
		embedIdx  int // index into batchTexts; -1 if no embedding needed
	}
	...
	for _, event := range parsed.Memory {
		ev := strings.ToUpper(event.Event)
		switch ev {
		case "ADD":
			if event.Text == "" {
				continue
			...
			if s.autoModel == "" && s.embedder != nil {
				idx = len(batchTexts)
				batchTexts = append(batchTexts, event.Text)
			}
			actions = append(actions, pendingAction{event: event, embedIdx: idx})
		...
		case "UPDATE":
			...
			if _, ok := idMap[intID]; !ok || event.Text == "" {
		...
		case "DELETE", "NOOP", "NONE":
			actions = append(actions, pendingAction{event: event, embedIdx: -1})
	...
	if len(batchTexts) > 0 && s.autoModel == "" && s.embedder != nil {
		var embedErr error
		batchEmbeddings, embedErr = s.embedder.EmbedBatch(ctx, batchTexts)
		if embedErr != nil {
			slog.Warn("reconcile: batch embed failed, actions will proceed without embeddings", "err", embedErr, "count", len(batchTexts))
			batchEmbeddings = nil
		}
		batchTexts = nil // release for GC
	}

	// Execute each action using pre-computed embeddings.
	...
	for _, action := range actions {
		...
		case "ADD":
			var embedding []float32
			if batchEmbeddings != nil && action.embedIdx >= 0 && action.embedIdx < len(batchEmbeddings) {
				embedding = batchEmbeddings[action.embedIdx]
			}
			newID, addErr := s.addInsightWithEmbedding(ctx, agentName, agentID, sessionID, event.Text, embedding)
			if addErr != nil {
		...
		case "UPDATE":
			...
			newID, updateErr := s.updateInsightWithEmbedding(ctx, agentName, agentID, sessionID, realID, event.Text, embedding)
			if updateErr != nil {
...
func (s *IngestService) gatherExistingMemories(ctx context.Context, agentID string, facts []string) ([]domain.Memory, error) {
	...
	// Pre-compute all fact embeddings in a single batch HTTP request to avoid
	// N sequential round-trips when using a local embedder (non-auto mode).
	var factVectors [][]float32
	if s.autoModel == "" && s.embedder != nil {
		var embedErr error
		factVectors, embedErr = s.embedder.EmbedBatch(ctx, facts)
		if embedErr != nil {
			slog.Warn("gatherExistingMemories: batch embed failed, vector leg will be skipped", "err", embedErr, "count", len(facts))
			factVectors = nil
		}
	}
	...
	for i, fact := range facts {
		...
		} else if factVectors != nil && i < len(factVectors) && factVectors[i] != nil {
			searchAttempts++
			var vecErr error
			vecMatches, vecErr = s.memories.VectorSearch(ctx, factVectors[i], filter, perFactLimit)
			if vecErr != nil {
	...
	// Release batch vectors after the loop; individual vectors inside are not retained.
	factVectors = nil
...
// When an embedder is available, all facts are batch-embedded in a single HTTP
// round-trip to eliminate per-fact network overhead.
func (s *IngestService) addAllFacts(ctx context.Context, agentName, agentID, sessionID string, facts []string) ([]string, int, error) {
	// Batch-embed all facts up front when possible.
	var embeddings [][]float32
	if s.autoModel == "" && s.embedder != nil {
		var err error
		embeddings, err = s.embedder.EmbedBatch(ctx, facts)
		if err != nil {
			slog.Warn("batch embed for addAllFacts failed, will create without embeddings", "err", err, "count", len(facts))
			embeddings = nil
		}
	}
...
// addInsight creates a new insight memory. Embeds the content if needed.
func (s *IngestService) addInsight(ctx context.Context, agentName, agentID, sessionID, content string) (string, error) {
	return s.addInsightWithEmbedding(ctx, agentName, agentID, sessionID, content, nil)
}

// addInsightWithEmbedding creates a new insight memory with a pre-computed embedding.
// If embedding is nil and an embedder is configured, it falls back to computing one.
func (s *IngestService) addInsightWithEmbedding(ctx context.Context, agentName, agentID, sessionID, content string, embedding []float32) (string, error) {
	if len(embedding) == 0 && s.autoModel == "" && s.embedder != nil {
		var err error
		embedding, err = s.embedder.Embed(ctx, content)
		if err != nil {
...
// updateInsight archives the old memory and creates a new one atomically. Embeds the content if needed.
func (s *IngestService) updateInsight(ctx context.Context, agentName, agentID, sessionID, oldID, newContent string) (string, error) {
	return s.updateInsightWithEmbedding(ctx, agentName, agentID, sessionID, oldID, newContent, nil)
}

// updateInsightWithEmbedding archives the old memory and creates a new one atomically
// (append-new + archive-old model) with a pre-computed embedding.
// If embedding is nil and an embedder is configured, it falls back to computing one.
func (s *IngestService) updateInsightWithEmbedding(ctx context.Context, agentName, agentID, sessionID, oldID, newContent string, embedding []float32) (string, error) {
