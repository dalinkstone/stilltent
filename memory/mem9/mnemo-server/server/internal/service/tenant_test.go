...
func TestBuildMemorySchema(t *testing.T) {
	...
	t.Run("no auto-model uses plain VECTOR(1536)", func(t *testing.T) {
		schema := tenant.BuildMemorySchema("", 0, 1536)
	...
	t.Run("no auto-model with custom clientDims", func(t *testing.T) {
		schema := tenant.BuildMemorySchema("", 0, 768)
		if !strings.Contains(schema, "VECTOR(768)") {
			t.Fatal("schema missing VECTOR(768) for custom clientDims")
		}
	})

	t.Run("auto-model emits EMBED_TEXT generated column with correct dims", func(t *testing.T) {
		schema := tenant.BuildMemorySchema("tidbcloud_free/amazon/titan-embed-text-v2", 1024, 1536)
		for _, needle := range commonChecks {
			if !strings.Contains(schema, needle) {
				t.Fatalf("schema missing %q", needle)
			}
...
func TestProvisionRejectsNonTiDBBackend(t *testing.T) {
	...
	svc := NewTenantService(nil, nil, pool, nil, "", 0, 1536, false)
...
func TestBuildDB9MemorySchema(t *testing.T) {
	...
	t.Run("no auto-model uses plain VECTOR(1536)", func(t *testing.T) {
		schema := tenant.BuildDB9MemorySchema("", 0, 1536)
		for _, needle := range commonChecks {
			if !strings.Contains(schema, needle) {
				t.Fatalf("schema missing %q", needle)
			}
	...
	t.Run("auto-model emits EMBED_TEXT generated column with correct dims", func(t *testing.T) {
		schema := tenant.BuildDB9MemorySchema("amazon.titan-embed-text-v2:0", 1024, 1536)
		for _, needle := range commonChecks {
			if !strings.Contains(schema, needle) {
				t.Fatalf("schema missing %q", needle)
			}
	...
	t.Run("auto-model with 512 dims", func(t *testing.T) {
		schema := tenant.BuildDB9MemorySchema("some-model", 512, 1536)
	...
	t.Run("single-quote in model name is escaped", func(t *testing.T) {
		schema := tenant.BuildDB9MemorySchema("model'inject", 1024, 1536)
...
func TestBuildMemorySchema_DimensionsArg(t *testing.T) {
	...
	t.Run("single-quote in model name is escaped", func(t *testing.T) {
		schema := tenant.BuildMemorySchema("model'inject", 1024, 1536)
		if !strings.Contains(schema, "model''inject") {
			t.Fatal("single quote in model name not escaped")
		}
	})
